...
class XianFeng5Page extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    return XianFeng5State();
  }
}

class XianFeng5State extends State<XianFeng5Page>
  ...
  void _getData() async {
    String url = _isSearch
        ? '${ApiConstant.xianFeng5Url}/index.php?s=vod-search-wd-$_currentKey-p-${_page}.html'
        : "${ApiConstant.xianFeng5Url}${_currentKey}-p-$_page.html";
    String response = await PornHubUtil.getHtmlFromHttpDeugger(url,isMobile: false);
    var doc = parse.parse(response);
    try {
      var tdElements = doc
          .getElementsByClassName('listcont')
          .first
      ...
      for (var value in tdElements) {
        ...
        if (aEles.length > 0) {
          ...
          String href = ApiConstant.xianFeng5Url + aEle.attributes['href'];
          var imgEle = aEle.getElementsByTagName('img').first;
          item.title = imgEle.attributes['alt'];
          item.imageUrl = ApiConstant.xianFeng5Url + imgEle.attributes['src'];
          item.targetUrl = href;
          _data.add(item);
        }
      }
      if (_btns == null) {
        _btns = [];
        var menu = doc.getElementsByClassName('navList').first;
  ...
  void goToDetail(VideoListItem data) async {
    ...
    movieBean.info = doc.getElementsByClassName('descont').first.text;
    movieBean.imgUrl = data.imageUrl;
    var listEles = doc
        .getElementsByClassName('playurl')
        .first
        .getElementsByTagName('p');
    List<MovieItemBean> list = [];
    for (var value in listEles) {
      MovieItemBean movieItemBean = MovieItemBean();
      movieItemBean.targetUrl = value.text;
      movieItemBean.name = '先锋';
