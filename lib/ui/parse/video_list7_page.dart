...
class VideoList7Page extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    return VideoList7State();
  }
}
...
class VideoList7State extends State<VideoList7Page>
  ...
  void _getData() async {
    String url = _isSearch
        ? '${ApiConstant.videoList7Url}/vodsearch/${Uri.encodeComponent(_currentKey)}----------$_page---.html'
        : "${ApiConstant.videoList7Url}${_page == 1?_currentKey:'$_currentKey/index-$_page.html'}";
    ...
    try {
      ...
      for (var value in tdElements) {
        ...
        if (aEles.length > 0) {
          ...
          item.imageUrl = item.imageUrl == null?imgEle.attributes['src']:item.imageUrl;
          item.targetUrl = href;
          _data.add(item);
        }
      }
      if (_btns == null) {
        _btns = [];
        var menu = doc.getElementsByClassName('dwkpxa kqiorvyrc3').first;
