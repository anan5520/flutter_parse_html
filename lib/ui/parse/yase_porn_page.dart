...
class YaSePornPage extends StatefulWidget {
  @override
  State<StatefulWidget> createState() {
    return YaSePornState();
  }
}

class YaSePornState extends State<YaSePornPage>
  ...
  String _currentKey = '/index.php/vod/show/id/21';
  ...
  void goToPlay(VideoListItem data) async {
    ...
    try {
      ...
      var gotoJs = doc.getElementsByClassName("videoPlayContainer").first.text;
      var playUrls = gotoJs.split(RegExp(r'url":"|.m3u8"'));
      var playUrl = '${playUrls[1]}.m3u8'.replaceAll('\\', '');
  ...
  void _getData() async {
    String url = _isSearch
        ? '${ApiConstant.yaSeUrl}/index.php/vod/search/page/$_page/wd/$_currentKey.html'
        : "${ApiConstant.yaSeUrl}$_currentKey/page/$_page.html";
    String response = await PornHubUtil.getHtmlFromHttpDeugger(url);
    _refreshController.refreshCompleted();
    _refreshController.loadComplete();
    var doc = parse.parse(response);
    try {
      var listEle = doc.getElementsByClassName('colVideoList');
      for (var value in listEle) {
        var aEles = value.getElementsByTagName('a');
        if (aEles.length > 0) {
          var aEle = aEles.first;
          VideoListItem item = VideoListItem();
          var hrefs = aEle.attributes['href'].replaceAll('.html', '');
          String href = '${ApiConstant.yaSeUrl}$hrefs';
          var imgEle = value.getElementsByClassName('img lazy').first;
