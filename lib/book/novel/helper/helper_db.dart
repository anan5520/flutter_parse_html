...
import 'package:flutter_parse_html/book/base/db/manager_db.dart';
...
class DBHelper extends BaseDBProvider {
  ...
  static const String COLUMN_ISLSJ = "lsj";
  ...
  Future<List<NovelBookInfo>> getAllBooks() async {
    List<NovelBookInfo> books = [];
    Database db = await getDB();
    List<Map<String, dynamic>> maps = await db.query(TABLE_NAME,where: '$COLUMN_ISLSJ = ?',whereArgs: [HomePage.inLsj?1:0]);
    if (maps.isNotEmpty) {
      for (Map<String, dynamic> map in maps) {
        NovelBookInfo book = NovelBookInfo.fromDBMap(map);
        books.add(book);
