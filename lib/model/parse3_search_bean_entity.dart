...
class Parse3SearchBeanData {
	...
	Parse3SearchBeanData.fromJson(Map<String?, dynamic> json) {
		pages = json['pages'];
		if (json['items'] != null) {
			items = [];(json['items'] as List).forEach((v) { items!.add(new Parse3SearchBeanDataItem.fromJson(v)); });
		}
	}
...
class Parse3SearchBeanDataItem {
	...
	Parse3SearchBeanDataItem.fromJson(Map<String?, dynamic> json) {
		...
		id = json['id'];
		if (json['tags'] != null) {
			tags = [];(json['tags'] as List).forEach((v) { tags!.add(new Parse3SearchBeanDataItemsTag.fromJson(v)); });
		}
	}
