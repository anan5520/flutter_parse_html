...
class XianFengBeanData {
	...
	XianFengBeanData.fromJson(Map<String?, dynamic> json) {
		pages = json['pages'];
		if (json['items'] != null) {
			items = [];(json['items'] as List).forEach((v) { items!.add(new XianFengBeanDataItem.fromJson(v)); });
		}
	}
...
class XianFengBeanDataItem {
	...
	XianFengBeanDataItem.fromJson(Map<String?, dynamic> json) {
		...
		if (json['areas'] != null) {
			areas = [];(json['areas'] as List).forEach((v) { areas!.add(new XianFengBeanDataItemsArea.fromJson(v)); });
		}
		id = json['id'];
	}
